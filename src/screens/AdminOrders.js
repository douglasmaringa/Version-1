...
const find=()=>{
    db.collection("order").where("delivered", "==", false).where("name", "==", search).orderBy('timestamp', 'desc').onSnapshot(querySnapshot=>{
        setData(querySnapshot.docs.map(doc=>({ ...doc.data(), id: doc.id })))
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="mb-4" style={{"display":"flex"}}>
                            ...
                            <input type="text" placeholder="Enter Customer Name" class="form-control mr-2" id="exampleInputPassword1"  value={search} onChange={(e)=>{setSearch(e.target.value)}}/>
                            <button onClick={find} class="btn btn-facebook btn-sm " >Search</button>
                        </div>
